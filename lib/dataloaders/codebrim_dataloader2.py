...
class CODEBRIM():
    ...
    def __init__(self, batch_size, operators=[], save_disk=False, p=1,contrastive= False):
        self.num_classes = 6
    ...
    def createandload_Dataset(self, root, static_transforms, dynamic_transforms, target_transforms, save_disk, p=1):
        if save_disk:
            key = ""
        ...
        if self.contrastive:
            return Contrastive_Tensor_Dataset_From_PIL(inputs, targets, dynamic_transforms, target_transforms)
        else:
            return Tensor_Dataset_From_PIL(inputs, targets, dynamic_transforms, target_transforms)
    ...
    def get_dataset(self, patch_size, p):
        ...
        transforms_train = transforms.Compose(
            [  # RandomCropNP(patch_size,patch_size),#transforms.RandomCrop(patch_size),
                RandomCropNP(patch_size, patch_size),
                RandomHorizontalFlipNP(),  # transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
                ToFloatTensor()])
        transforms_test_static = transforms.Compose([PIL2NP(), ResizeNP_smallside(patch_size),
                                                     CenterCropNP(patch_size, patch_size)
                                                     ] + self.operators + [
                                                        transforms.ToTensor(),
                                                        ToFloatTensor()])
        ...
        train_set = self.createandload_Dataset(os.path.join(self.dataset_path, 'train'),
                                               transform_train_static, transforms_train, self.target_transforms,
                                               self.save_disk, p)
        val_set = self.createandload_Dataset(os.path.join(self.dataset_path, 'val'),
                                             transforms_test_static, transforms_test, self.target_transforms,
                                             self.save_disk)
...
if __name__=="__main__":
    ...
    c_dataset = CODEBRIM(4,contrastive=True)
    anchor,positive_example,neg_example = next(iter(c_dataset.trainloader))
    print('shapes',anchor.shape,positive_example.shape,neg_example.shape)
    for idx in range(anchor.shape[0]):
        ...
        plt.savefig(f"./view_examples/anchor_codebrim_examples{idx}.png")

        img = positive_example[idx][0]
        print(img.shape)
        plt.Figure()

        plt.imshow(img.permute(1, 2, 0).detach().cpu().numpy())
        plt.savefig(f"./view_examples/pos_codebrim_examples{idx}.png")

        img = neg_example[idx][0]
        print(img.shape)
        plt.Figure()

        plt.imshow(img.permute(1, 2, 0).detach().cpu().numpy())
        plt.savefig(f"./view_examples/neg_codebrim_examples{idx}.png")
