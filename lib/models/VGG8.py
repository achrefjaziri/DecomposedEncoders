...
class HingeLoss(nn.Module):
    ...
    def forward(self, input, positive):
        ...
        loss = 0.5 * (torch.mean(input[positive, positive]) + (
                    torch.sum(input) - torch.sum(input[positive, positive])) / (
                                  (positive.size(0) - 1) * (positive.size(0))))
        return loss
    ...
    def forward(self, input):
        ...
        for k in self.k:
            ...
            for time_pred in input:
                ...
                acc_k.append(torch.mean(torch.tensor([(index == input_line).any().float() for (index, input_line) in
                                                      enumerate(torch.topk(time_pred, k, dim=1)[1])])))
...
class Loss(nn.Module):
    ...
    def forward(self, input):
        ...
        return (self.loss(first_score, positive) + self.loss(second_score, positive) + self.loss(third_score,
                                                                                                 positive)) / 3, [
                   first_score.detach(), second_score.detach(), third_score.detach()]
...
class VGG_8(nn.Module):
    def __init__(self,in_channels, temp):
    ...
    def forward(self, input,all_layers=False):
        convs = nn.ModuleList([self.conv1, self.conv2, self.conv3, self.conv4,self.conv5,self.conv6])
        ...
        for i, block in enumerate(self.blocks):
                if all_layers:
                    conv_outs.append(convs[i](res))
                res = block(res)
                if all_layers:
                    all_outs.append(res)
        if all_layers:
            return res,all_outs,conv_outs
        else:
            return res
